...
import { LayoutDashboard, BookOpen, ClipboardList, Calendar, User, LogOut, Menu, X } from 'lucide-react';
      ...
      <main className="flex-1 flex flex-col overflow-hidden w-full relative">
        <header className="h-16 bg-white/70 backdrop-blur-xl border-b border-white/50 shadow-sm flex items-center justify-between px-4 sm:px-6 shrink-0 z-30">
          ...
          <div className="flex items-center space-x-3 sm:space-x-4">
            <span className="text-sm font-medium text-gray-500 hidden sm:block truncate max-w-[120px]">{user?.name}</span>
            <div className="h-8 w-8 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold shrink-0 shadow-sm border border-white">
              {user?.name?.charAt(0)}
            </div>
          </div>
        </header>
